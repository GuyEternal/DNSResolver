...
def main():
    msg = Message(
        Header(
            ...
            flags= QR.QUERY.value | OPCODE.QUERY.value | RD.RECURSION_NOT_DESIRED.value, # We take or of the values to set the flags as 100 | 001 => 101 so we can use the enum values directly and take or of them to set the flags 
            QDCOUNT=1, 
            ANCOUNT=0, 
            NSCOUNT=0, 
            ARCOUNT=0),
    ...
    response = send_udp_message(byte_string_to_send, '1.1.1.1', 53)
